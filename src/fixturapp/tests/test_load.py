...
class FixturappLoadTests(TestCase):
    ...
    def test_fixture_was_loaded_by_calling_command_for_one_app(self):
        """Check if the fixturapp_load command has loaded Dummy fixtures into database"""
    ...
    def test_raises_when_cant_find_datasets_package_for_app(self):
        """Raises when loading from a specific app that does not have datasets package"""
        self.assertRaises(ImportError, lambda: call_command('fixturapp_load', 'fixturapp.tests.emptyapp'))

    def test_raises_when_app_have_empty_datasets_package(self):
        """Raises when loading from a specific app that has an empty datasets package"""
        self.assertRaises(LookupError, lambda: call_command('fixturapp_load', 'fixturapp.tests.incompleteapp'))

    def test_raises_when_some_app_does_not_have_data(self):
        """``fixturapp_load app1 app2`` should be sucessfull only if all apps have data."""
        self.assertRaises(LookupError, 
            lambda: call_command('fixturapp_load', 'fixturapp.tests.dummyapp', 'fixturapp.tests.incompleteapp'))
        self.assertRaises(ImportError, 
            lambda: call_command('fixturapp_load', 'fixturapp.tests.dummyapp', 'fixturapp.tests.emptyapp'))
